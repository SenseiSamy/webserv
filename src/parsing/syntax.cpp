...
void route_value(route &route, const std::string &word)
{
	...
	if (word != "cgi" && value.size() != 1)
		throw std::runtime_error("Syntax error: Expected one value after " + word + " at line " + to_string(index_line));
	if (word == "cgi" && value.size() != 2)
		throw std::runtime_error("Syntax error: Expected two values after cgi at line " + to_string(index_line));
	...
	else if (word == "directory_listing")
	{
		if (value[0] != "true" && value[0] != "false")
			throw std::runtime_error("Syntax error: Invalid value for directory_listing at line " + to_string(index_line));
	...
	{
		if (value[0] != "true" && value[0] != "false")
			throw std::runtime_error("Syntax error: Invalid value for cgi_upload_enable at line " + to_string(index_line));
...
void server_value(server &server, const std::string &word, const std::map<unsigned short, std::string> &error_codes)
{
	...
	else if (word == "default_server")
	{
		if (value[0] == "true")
			...
			throw std::runtime_error("Syntax error: Invalid value for default_server at line " + to_string(index_line));
	}
	...
	else if (word == "error_pages")
	{
		...
		if (error_codes.find(code) == error_codes.end())
			throw std::runtime_error("Syntax error: Invalid error code " + value[0] + " at line " + to_string(index_line));
...
void syntax_brackets()
{
	...
	while (word != "")
	{
		if (word == "server" || word == "route")
		{
			word = next_word();
			if (word != "{")
				throw std::runtime_error("Syntax error: Expected '{' after " + word + "at line " + to_string(index_line));
			++open_brackets;
		}
		else if (word == "}")
			++close_brackets;
